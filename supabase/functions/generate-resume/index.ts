...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Generating resume for job info...');
...
Return the resume in clean text format with proper spacing and organization.`;
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'user', content: `Transform this job information into a professional Harvard format resume:\n\n${jobInfo}` }
        ],
      }),
    });
